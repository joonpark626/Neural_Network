...
def error_checking(calculated_outputs, data_outputs):
    ...
    Output_size = len(data_outputs[0])
    Tr_size = len(data_outputs)
    Y_max_min = max(data_outputs) - min(data_outputs)
    # print("error_checking: Output_size = ", Output_size)
    # print("error_checking: Tr_size = ", Tr_size)
    ...
    error = np.square(np.subtract(calculated_outputs, data_outputs))
...
def delta_W_HL1_computation(activated_hidden1_outputs, data_inputs, W_HL1_size):
    ...
    delta_w_HL1 = np.array([[activated_hidden1_outputs[row]*data_inputs[col] for col in range(W_HL1_size[1])] for row in range(W_HL1_size[0])])
...
def de_E_function_HL1(W_out_new, W_HL1_new, B_out_new, B_HL1_new, calculated_outputs_Tr, activated_hidden1_outputs, data_inputs, data_outputs):
    ...
    if (len(W_out_new) == 1):
        ...
        for i in range(len(activated_hidden1_outputs)):
            de_w_HL1[i] = delta_W_HL1_computation(de_B_HL1[i], data_inputs[i], W_HL1_size)
...
if (Ali_num_HL == 1):
    ...
    [W_out_new, W_HL1_new, B_out_new, B_HL1_new, num_epoch_iter, Training_error_collection] = Conjugate_Gradient_HL1(user_Validation_error, 
                                                                                                                    W_out_Debug, W_HL1_Debug, B_out_Debug, B_HL1_Debug,  
                                                                                                                    data_Tr_inputs, Testing_data_output, max_epoch)
