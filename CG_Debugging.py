...
def feedforward_comp(W_out_new, W_HL1_new, B_out_new, B_HL1_new, data_inputs):
    ...
    print("feedforward_comp: np.shape(data_inputs) = ", np.shape(data_inputs))
    print("feedforward_comp: np.shape(W_HL1_new) = ", np.shape(W_HL1_new))
    HL1_comp = np.dot(data_inputs, np.transpose(np.array(W_HL1_new)))
    print("feedforward_comp: HL1_comp = ", HL1_comp)
    print("feedforward_comp: np.transpose(HL1_comp) = ", np.transpose(HL1_comp))
    print("\n")
    ...
    Activated_SUM_HL1_comp = np.array([transfer_sigmoid(x) for x in deepcopy(SUM_HL1_comp)])
    Activated_SUM_HL1_comp = Activated_SUM_HL1_comp.transpose()
    print("feedforward_comp: Activated_SUM_HL1_comp = ", Activated_SUM_HL1_comp)
    print("\n")
    
    print("feedforward_comp: np.shape(W_out_new) = ", np.shape(W_out_new))
    print("feedforward_comp: np.shape(Activated_SUM_HL1_comp) = ", np.shape(Activated_SUM_HL1_comp))
    dot_final_calculated_outputs = np.dot(np.transpose([Activated_SUM_HL1_comp]), W_out_new)
    add_final_calculated_outputs = np.add(dot_final_calculated_outputs, B_out_new)
    print("feedforward_comp: dot_final_calculated_outputs = ", dot_final_calculated_outputs)
    print("feedforward_comp: add_final_calculated_outputs = ", add_final_calculated_outputs)
    print("\n")
...
def Conjugate_Gradient_HL1(user_Validation_error, W_out, W_HL1, B_out, B_HL1, data_inputs, data_outputs, max_epoch):
    ...
    for epoch in range(max_epoch):
        ...
        calculated_outputs_Tr, activated_hidden1_outputs_Tr = feedforward_comp(W_out_new[0], W_HL1_new[0], B_out_new[0], B_HL1_new[0], data_inputs)
        FF_error_Tr = error_checking(calculated_outputs_Tr, data_outputs)
        
        print("Conjugate_Gradient_HL1 : FF_error_Tr = ", FF_error_Tr)
        print("Conjugate_Gradient_HL1 : calculated_outputs_Tr = ", calculated_outputs_Tr)
        print("Conjugate_Gradient_HL1 : activated_hidden1_outputs_Tr = ", activated_hidden1_outputs_Tr)
        print("\n")
...
for sample in data:
    for j in range(len(sample)):
        if (j < Ali_num_inputs):
            temp_data_inputs.extend([sample[j]])

        else:
            temp_data_outputs.extend([sample[j]])
    data_inputs.append(deepcopy(temp_data_inputs))
    data_outputs.append(deepcopy(temp_data_outputs))
    temp_data_inputs.clear()
    temp_data_outputs.clear()
...
if (Ali_num_HL == 1):
    ...
    print("\n")
    [W_HL1_new, B_HL1_new, W_out_new, B_out_new, num_epoch_iter, Training_error_collection, FF_error_Te] = Conjugate_Gradient_HL1(user_Validation_error, W_out, W_HL1, B_out, B_HL1, data_inputs, data_outputs, max_epoch)
elif (Ali_num_HL == 2):
